...
def _date_range_for_omie(data_emis: Optional[datetime]) -> Tuple[str, str]:
    ...
    Usa um intervalo de 3 dias para trás e 1 para frente para garantir a captura.
    """
    if data_emis:
...
async def _ensure_tables(conn: asyncpg.Connection) -> None:
    await conn.execute("""
    CREATE TABLE IF NOT EXISTS public.omie_webhook_events (id bigserial PRIMARY KEY, source text, event_type text, route text, http_status integer, topic text, status text, raw_headers jsonb, payload jsonb, event_ts timestamptz, received_at timestamptz DEFAULT now(), processed boolean DEFAULT false, processed_at timestamptz, error text);""")
    await conn.execute("""
    CREATE TABLE IF NOT EXISTS public.omie_pedido (id_pedido_omie bigint PRIMARY KEY, numero text, valor_total numeric, situacao text, quantidade_itens integer, cliente_codigo text, detalhe jsonb, created_at timestamptz DEFAULT now(), updated_at timestamptz);""")
    await conn.execute("""
    CREATE TABLE IF NOT EXISTS public.omie_nfe (chave_nfe text PRIMARY KEY, numero text, serie text, emitida_em timestamptz, cnpj_emitente text, cnpj_destinatario text, valor_total numeric(15,2), status text, xml text, xml_url text, danfe_url text, last_event_at timestamptz, updated_at timestamptz, recebido_em timestamptz DEFAULT now(), raw jsonb);""")

# ------------------------------------------------------------------------------
# HEALTH (sem alterações)
# ------------------------------------------------------------------------------
@router.get("/healthz")
async def healthz():
    try:
...
async def pedidos_webhook(request: Request, token: str = Query(...)):
    ...
    async with app.state.pool.acquire() as conn:
        await conn.execute("INSERT INTO public.omie_webhook_events (source,event_type,route,http_status,topic,raw_headers,payload,event_ts,received_at,processed) VALUES ('omie','omie_webhook_received','/omie/webhook',200,$1,$2,$3,now(),now(),FALSE);", _safe_text(_pick(body, "topic")) or "pedido", json.dumps(dict(request.headers), ensure_ascii=False, default=str), json.dumps(body, ensure_ascii=False, default=str))
...
async def nfe_webhook(request: Request, token: str = Query(...)):
    ...
    body = await request.json()
    async with app.state.pool.acquire() as conn:
        await conn.execute("INSERT INTO public.omie_webhook_events (source,event_type,route,http_status,topic,raw_headers,payload,event_ts,received_at,processed) VALUES ('omie','nfe_xml_received','/xml/omie/webhook',200,$1,$2,$3,now(),now(),FALSE);", _safe_text(_pick(body, "topic")) or "nfe", json.dumps(dict(request.headers), ensure_ascii=False, default=str), json.dumps(body, ensure_ascii=False, default=str))
    return JSONResponse({"ok": True, "received": True})

# ------------------------------------------------------------------------------
# Processamento de NF-e (LÓGICA CORRIGIDA)
# ------------------------------------------------------------------------------
async def processar_nfe(conn: asyncpg.Connection, payload: Dict[str, Any], received_at: datetime, client: httpx.AsyncClient) -> bool:
    """
    Usa a data de recebimento do webhook como base para a busca na API Omie.
    """
    try:
        ...
        # 1. Tenta pegar a data de emissão de dentro do webhook (ideal)
        data_base = _parse_dt(_pick(event_payload, "data_emis", "dh_emis", "dEmissao"))

        # 2. Se não encontrar, usa a data que o evento CHEGOU (sua sugestão!)
        if not data_base:
            data_base = received_at
            logger.info(f"📅 Data não encontrada no payload. Usando data de recebimento do evento: {data_base.strftime('%d/%m/%Y')}")
        
        # Monta a chamada para buscar o bloco de dados na API
        dEmiInicial, dEmiFinal = _date_range_for_omie(data_base)
        ...
        response_data = await _omie_post_with_retry(client, OMIE_XML_URL, OMIE_XML_LIST_CALL, api_payload)
        ...
        for doc in documentos:
            ...
            data_emissao_nf = _parse_dt(doc.get("dEmissao"))
            xml_str = doc.get("cXml")
            json_completo_str = json.dumps(doc, ensure_ascii=False, default=str)
            
            await conn.execute("""
                INSERT INTO public.omie_nfe (chave_nfe, numero, serie, status, valor_total, emitida_em, xml, raw, last_event_at, updated_at, recebido_em)
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8, now(), now(), now())
                ON CONFLICT (chave_nfe) DO UPDATE SET
                    numero = EXCLUDED.numero, serie = EXCLUDED.serie, status = EXCLUDED.status,
                    valor_total = EXCLUDED.valor_total, emitida_em = EXCLUDED.emitida_em,
                    xml = EXCLUDED.xml, raw = EXCLUDED.raw, updated_at = now();
            """, chave_nfe, numero_nf, serie_nf, status_nf, valor_nf, data_emissao_nf, xml_str, json_completo_str)
            
            logger.info(f"✔️ NF-e Chave: {chave_nfe} salva/atualizada na tabela omie_nfe.")
...
# ------------------------------------------------------------------------------
# Pedidos helpers (sem alterações)
# ------------------------------------------------------------------------------
...
async def run_jobs(secret: str = Query(...)):
    ...
    try:
        async with app.state.pool.acquire() as conn:
            # --- AJUSTE AQUI: buscando também a coluna 'received_at' ---
            rows = await conn.fetch("""
                SELECT id, route, payload, received_at
            ...
            async with httpx.AsyncClient(timeout=OMIE_TIMEOUT_SECONDS) as client:
                for r in rows:
                    ...
                    received_at = r["received_at"] # Pegando a data de recebimento

                    try:
                        logger.info(f"⚡ Processando evento {ev_id} da rota {route}")

                        if "/xml/omie/webhook" in route:
                            # --- AJUSTE AQUI: passando 'received_at' para a função ---
                            ok = await processar_nfe(conn, payload, received_at, client)
                            if ok:
                                ...
                                try:
                                    ...
                                    await conn.execute("""
                                        ...
                                        ON CONFLICT (id_pedido_omie) DO UPDATE SET
                                            numero = COALESCE(EXCLUDED.numero, omie_pedido.numero), valor_total = COALESCE(EXCLUDED.valor_total, omie_pedido.valor_total),
                                            situacao = COALESCE(EXCLUDED.situacao, omie_pedido.situacao), quantidade_itens = COALESCE(EXCLUDED.quantidade_itens, omie_pedido.quantidade_itens),
                                            cliente_codigo = COALESCE(EXCLUDED.cliente_codigo, omie_pedido.cliente_codigo), detalhe = COALESCE(EXCLUDED.detalhe, omie_pedido.detalhe),
                                            updated_at = now();
                                    """, codigo_pedido, numero, valor_total, situacao, quantidade_itens, cliente_codigo, json.dumps(pedido_venda, ensure_ascii=False, default=str))
...
async def debug_events(secret: str = Query(...)):
    ...
    async with app.state.pool.acquire() as conn:
        events = await conn.fetch("SELECT id, route, processed, error, received_at, payload::text AS payload_text FROM public.omie_webhook_events ORDER BY id DESC LIMIT 20")
    ...
    return JSONResponse({"events": [dict(e) for e in events], "pedidos": [dict(p) for p in pedidos], "nfes": [dict(n) for n in nfes]})

@router.get("/admin/debug/tokens")
async def debug_tokens(secret: str = Query(...)):
    if secret != ADMIN_RUNJOBS_SECRET:
        raise HTTPException(status_code=403, detail="invalid secret")
    return JSONResponse({"WEBHOOK_TOKEN_PED": WEBHOOK_TOKEN_PED, "WEBHOOK_TOKEN_XML": WEBHOOK_TOKEN_XML, "OMIE_APP_KEY": "CONFIGURADO" if OMIE_APP_KEY else "NÃO CONFIGURADO", "OMIE_APP_SECRET": "CONFIGURADO" if OMIE_APP_SECRET else "NÃO CONFIGURADO", "DATABASE_URL": "CONFIGURADO" if DATABASE_URL else "NÃO CONFIGURADO"})
