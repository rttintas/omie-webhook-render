...
def _extract_pedido_id_from_event(payload: Dict[str, Any]) -> Optional[int]:
    try:
        ...
        if ev.get("idPedido"): return int(ev["idPedido"])
        for k in ("codigo_pedido","id_pedido","pedido_id","nCodPed"):
            if ev.get(k): return int(ev[k])
    except Exception:
        pass
                    ...
                    try:
                        ...
                        if "/xml/omie/webhook" in route:
                            ...
                            if ok:
                                ...
                                errors += 1
                                await conn.execute("UPDATE public.omie_webhook_events SET error=$1 WHERE id=$2",
                                                   "processar_nfe_failed", ev_id)
                            continue
